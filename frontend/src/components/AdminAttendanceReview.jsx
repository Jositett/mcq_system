 ...
 * Allows admins to review, approve, or reject flagged or all attendance check-ins.
 * Optionally shows face match confidence and captured image (if available from backend).
 */
export default function AdminAttendanceReview({ token }) {
  ...
  useEffect(() => {
    async function fetchRecords() {
      ...
      try {
        const res = await axios.get(`${API_URL}/attendance/review`, {
  ...
  return (
    <div className="bg-white rounded shadow p-6 w-full max-w-2xl mx-auto mt-6">
      <h2 className="text-xl font-semibold mb-4">Admin Attendance Review</h2>
